...
// liukai added@2012.11.28 for button backlight. BEGIN
...
// liukai added@2012.11.27 for button backlight. BEGIN
#define TS_ACTION_UP		1
#define TS_ACTION_DOWN	0
#define B_BACKLIGHT_ON	1
#define B_BACKLIGHT_OFF	0

#define BUTTON_OFF_DELAY		3500		// 3.5 s

static int ts_status = TS_ACTION_UP;
static int button_backlight_status = B_BACKLIGHT_OFF;
static pthread_t button_check;
static pthread_once_t button_init = PTHREAD_ONCE_INIT;
static pthread_mutex_t button_lock = PTHREAD_MUTEX_INITIALIZER;
...
// liukai added@2012.11.27 for button backlight. BEGIN
void init_button_lock(void)
{
    pthread_mutex_init(&button_lock, NULL);
}

int button_control(int code)
{
	int fd,ret;
	char msg[1] = {0}; 
	...
	//check open failed
	if (fd < 0){
		//ALOGE("Failed to open socket, err code %d\n", fd);
		close(fd);
		return -1;
	}
	...
	if(ret < 0){
		//ALOGE("Failed to write socket, err code %d\n", ret);
		close(fd);
		return -2;
	}else{
		if(code == 1)   // on
			button_backlight_status = B_BACKLIGHT_ON;
		else if(code == 0)
			button_backlight_status = B_BACKLIGHT_OFF;
	}
	close(fd);
	return 0;
}

void *button_timer(void *arg)
{
	int button_timer_tick = 0;
	
	thread_count++;

	//ALOGD("Enter button_timer\n");
	
	while(ts_status == TS_ACTION_UP && button_timer_tick < BUTTON_OFF_DELAY){
		button_timer_tick +=5;
		usleep(5000);	// 5ms step
	}
	
	if (ts_status == TS_ACTION_DOWN)
		goto out;

	if (button_timer_tick >= BUTTON_OFF_DELAY){
		//ALOGD("Time's up!BUTTON BACKLIGHT TURN OFF !!!!!\n");
		button_control(0);
	}

out:
	//ALOGD("Exit button_timer\n");
...
void start_button_timer()
{
	...
	if(thread_count == 0){
		...
		if (pthread_create(&button_check, NULL, button_timer, NULL) != 0){
			ALOGE("Can't creat button_timer pthread\n");
			pthread_mutex_unlock(&button_lock);
		}
		pthread_mutex_unlock(&button_lock);
	}else if(thread_count == 1)
		;	//ALOGV("timer thread is alive now.\n");
	else
		ALOGE("error state from thread.\n");
}

void process_ud_action(RawEvent* buffer)
{
...
ACTION_UP:
	//ALOGE("\nup action\n");
	ts_status = TS_ACTION_UP;
	start_button_timer();
	return;

ACTION_DOWN:
	//ALOGE("\ndown action\n");
	ts_status = TS_ACTION_DOWN;
	if(button_backlight_status == B_BACKLIGHT_OFF){
		button_control(1);
	}
	return;
...
// liukai added@2012.11.27 for button backlight. BEGIN
    pthread_once(&button_init, init_button_lock);
// END
