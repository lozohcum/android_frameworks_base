...
final class Settings {
    ...
    boolean disableSystemPackageLPw(String name) {
        ...
        // always make sure the system package code and resource paths dont
        // change
        if (dp == null) {
            ...
            for (BasePermission bp : permissions.values()) {
                if (origPkg.equals(bp.sourcePackage)) {
                    if (PackageManagerService.DEBUG_UPGRADE)
                        Log.v(PackageManagerService.TAG,
    ...
    private PackageSetting getPackageLPw(String name, PackageSetting origPackage,
        ...
        if (p != null) {
            if (!p.codePath.equals(codePath)) {
                ...
                if ((p.pkgFlags & ApplicationInfo.FLAG_SYSTEM) != 0) {
                    // This is an updated system app with versions in both
                    // system
                     ...
                     * native library path over the one stored in the package
                     * settings since we might have moved from internal to
                     * external storage or vice versa.
                     */
        ...
        if (p == null) {
            ...
            // of code path mismatch or user id mismatch of an updated system
            // partition
            if (!create) {
                return null;
            }
            if (origPackage != null) {
                ...
                if (PackageManagerService.DEBUG_UPGRADE)
                    Log.v(PackageManagerService.TAG, "Package "
    ...
    /*
     * Update the shared user setting when a package using specifying the shared
     * user id is removed. The gids associated with each permission of the
     * deleted package are removed from the shared user's gid list only if its
     * not in use by other permissions of packages in the shared user setting.
     */
    ...
    // Legacy reader, used to read in the old file format after an upgrade. Not
    // used after that.
    void readStoppedLPw() {
        FileInputStream str = null;
    ...
    void writeLPr() {
        // Debug.startMethodTracing("/data/system/packageprof", 8 * 1024 *
        // 1024);
        ...
        try {
            ...
            try {
                ...
                for (final PackageSetting pkg : mPackages.values()) {
                    ...
                    //
                    // NOTE: We prefer not to expose all ApplicationInfo flags
                    // for now.
                    //
                    // DO NOT MODIFY THIS FORMAT UNLESS YOU CAN ALSO MODIFY ITS
                    // USERS
                    // FROM NATIVE CODE. AT THE MOMENT, LOOK AT THE FOLLOWING
                    // SOURCES:
    ...
    private void readDisabledSysPackageLPw(XmlPullParser parser) throws XmlPullParserException,
        ...
        while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
            ...
            } else if (tagName.equals("revoked-perms")) {// song start
