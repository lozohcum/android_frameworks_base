...
// liukai added@2012.12.03 for button backlight switch
import static android.provider.Settings.System.BUTTON_BACKLIGHT_SWITCH;
// end
...
// liukai added@2012.11.29 begin
    ...
    private static int mButtonBacklightSwitch = 0;
// end
    /*
    ...
    */
    ...
    /**
     * liukai added@2012.11.28 for our own button-backlight way
     * Function name	:	turnOnButtonBacklight
     * Function		:	turn on the button-backlight
     * mLightSensorButtonBrightness   (light sensor brightness)
     */
    private void turnOnButtonBacklight() {
	if (mButtonBacklightSwitch == 0)
	    mButtonLight.setBrightness(mLightSensorButtonBrightness > 0 ?
                mLightSensorButtonBrightness : 255);
    }

    /**
     * liukai added@2012.11.28 for our own button-backlight way
     * Function name	:	turnOffButtonBacklight
     * Function		:	turn off the button-backlight
     */
    private void turnOffButtonBacklight() {
	    mButtonLight.setBrightness(0);
    }

    /**
     * liukai added@2012.12.04 for our own button-backlight way
     * Function name	:	initButtonSocket
     * Function		:	init "button-socket".
     */
    private void initButtonSocket() {
	try{
		mServerSocket = new LocalServerSocket("button-socket");
	}catch (IOException e) {
		Log.v(TAG, "in initButtonSocket" + e);
		return;
	}
    }

    /**
     * liukai added@2012.11.29 for our own button-backlight way
     * Function name	:	startButtonThread
     * Function		:	start "button-socket" for connecting between EventHub and this.
     */
    private void startButtonThread() {
		Thread t = new Thread() { 
			@Override public void run() {
				LocalSocket socket = null;
				//Log.v(TAG, "Start startButtonThread");
				while (mButtonBacklightSwitch == 0) {
					try {
						...
						if (socket != null) {
							...
							int size = input.read(bytes, 0, 1);
							String str = new String(bytes);
							// 1 0
							if(str != null) {
								if(str.equals("1"))
									turnOnButtonBacklight();
								else if(str.equals("0"))
									turnOffButtonBacklight();
								else
									;
							}
						} else {
							Log.v(TAG, "Unluckly button-socket died");
				...
				try {
					...
					Log.v(TAG, "closing error: " + e);
				}
				Log.v(TAG, "Stop startButtonThread");
			}
		};
	    t.start();
    }
	...
	//liukai added@2012.12.04 for our own button-backlight way begin
	initButtonSocket();
	startButtonThread();
	//end
	...
	//liukai added@2012.12.04 for our own button-backlight way begin
	mButtonBacklightSwitch = Settings.System.getInt(mContext.getContentResolver(),
	...
	// liukai added@2012.12.03 for button backlight switch
        mContext.getContentResolver().registerContentObserver(Settings.System.getUriFor(
            Settings.System.BUTTON_BACKLIGHT_SWITCH), false,
            new ContentObserver(new Handler()) {
                public void onChange(boolean selfChange) {
                    mButtonBacklightSwitch = Settings.System.getInt(mContext.getContentResolver(),
                        Settings.System.BUTTON_BACKLIGHT_SWITCH, 0);
				if (mButtonBacklightSwitch == 0) {
					...
					initButtonSocket();
					startButtonThread();
				} else if (mButtonBacklightSwitch == 1)
					turnOffButtonBacklight();
				else
					Log.e(TAG, "Error code get from settings");
                }
            });
	// end
                ...
                public void handleMessage(Message msg) {
                    ...
                    if (msg.what == ANIMATE_LIGHTS) {
                        ...
                        if ((mask & BUTTON_BRIGHT_BIT) != 0) {
                            ...
                            // liukai hide this@2012.12.03 because we use our own button backlight setting
                            // this maybe insterrupt ours setting, so hide this.
    ...
    private void lightSensorChangedLocked(int value, boolean immediate) {
        ...
        if (mLightSensorValue != value) {
            ...
            if ((mPowerState & BATTERY_LOW_BIT) == 0) {
                ...
                // liukai hide this@2012.12.03 for using our own button backlight control
                // this maybe interrupt ours setting, so hide it.
