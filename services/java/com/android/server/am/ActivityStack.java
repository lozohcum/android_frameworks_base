...
final class ActivityStack {
    ...
    /* Values for Limit Mode
       Add by ShenDu, kunan.zhao */
    public ArrayList<String> mBlackList,mWhiteList;
    boolean mAppLimitEnable = false;
    ...
    final boolean realStartActivityLocked(ActivityRecord r,
        ...
        if (!acceptToStart(r.packageName)) {
            startUnlockActivity(r.packageName);
        }
    ...
    /* Show an Unlock Activity, 
       add by ShenDu, Zhao*/
    public void startUnlockActivity(String packageName){
        int mode = Settings.System.getInt(mContext.getContentResolver(), Settings.System.APP_LIMIT_PASSWORD_STYLE, 0);
        String unlockActivityName = (mode ==0 ? ".cyanogenmod.UnlockWithLimitPassword" : ".cyanogenmod.UnlockWithLimitPattern");
        Intent intent = new Intent();
        intent.setComponent(new ComponentName("com.android.settings", "com.android.settings" + unlockActivityName));
    ...
    /* Reload the limited applictaion list
       Add by ShenDu, kunan.zhao */
    private void reloadAppList() {
        try {
            mBlackList = new ArrayList<String>(Arrays.asList(Settings.System.getString(mContext.getContentResolver(),
                Settings.System.APP_LIMIT_LIST).split("\\|")));
        } catch (Exception e) {
    ...
    /* Check if a application limited to start
       Add by ShenDu, kunan.zhao */    
    private boolean acceptToStart(String pacakgeName) {
       if (Settings.System.getInt(mContext.getContentResolver(),
               Settings.System.APP_LIMIT_ENABLE, 0) == 1) {
            reloadAppList();
            if (mBlackList.contains(pacakgeName) && !mWhiteList.contains(pacakgeName)) {
                return false;
            } else {
                return true;
            }
