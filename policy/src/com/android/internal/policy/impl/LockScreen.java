...
import com.android.internal.widget.Ring.*;
import java.io.File;
 ...
 */
class LockScreen extends LinearLayout implements KeyguardScreen ,RotarySelector.OnDialTriggerListener{
    ...
    class RotarySelectorMethods implements RotarySelector.OnDialTriggerListener, UnlockWidgetCommonMethods{
		...
		public void cleanUp() {
			// TODO Auto-generated method stub
			mRotarySelector.setOnDialTriggerListener(null);
		}
...
//	            updateRightTabResources();
    ...
    //shendu MulitRingViewMethods
    // 控制接口
    class MulitRingViewMethods implements KeyguardStatusViewManager.ExtStatusChange,MulitRingView.OnTriggerListener, UnlockWidgetCommonMethods{

    	 private final  MulitRingView 	mMulitRingView;		

    	 MulitRingViewMethods( MulitRingView mulitRingView) {
    		 mMulitRingView = mulitRingView;
         }
         
         @Override
        public boolean onExtStatusChange(int type,boolean show,boolean extInfo,int level){
        	// 更新额外信息
        	return mMulitRingView.onExtStatusChange(type,show,extInfo,level);
        }
		...
		@Override	// view硬件加速
		public View getView() {
			return mMulitRingView;
		}

		@Override	// 状态重置
		public void reset(boolean animate) {
			mMulitRingView.reset(animate);
		}

		@Override	// 释放资源
		public void cleanUp() {
			mMulitRingView.cleanUp();
		}

		@Override	// 选择
		public void onTrigger(int func,String extern) {
			switch(func){
			case MulitRingView.OnTriggerListener.UNLOCK:
					mCallback.goToUnlockScreen();
					break;
			case MulitRingView.OnTriggerListener.SOUND:
                    toggleRingMode();
                    mCallback.pokeWakelock();
					break;
			case MulitRingView.OnTriggerListener.CAMERA:
					launchActivity(new Intent(MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA));
					mCallback.pokeWakelock();
					break;
			case MulitRingView.OnTriggerListener.SMS:
					//sms
					Intent intentSMS = new Intent();
					ComponentName compSMS = new ComponentName("com.android.mms","com.android.mms.ui.ConversationList");
        			intentSMS.setComponent(compSMS);
        			intentSMS.setAction("android.intent.action.VIEW");
					launchActivity(intentSMS);
					mCallback.pokeWakelock();
					break;
			case MulitRingView.OnTriggerListener.CALL:
					//call
					Intent intentCALL = new Intent();
					ComponentName compCALL = new ComponentName("com.android.contacts","com.android.contacts.activities.DialtactsActivity");
        			intentCALL.setComponent(compCALL);
        			intentCALL.setAction("android.intent.action.VIEW");
        			launchActivity(intentCALL);
					mCallback.pokeWakelock();
					break;
			case MulitRingView.OnTriggerListener.APP:
					Intent intent = new Intent();
					if(extern==null) return;
					int position = extern.lastIndexOf('.');
					if(position<=0) return;
					intent.setClassName(extern.substring(0,position-1),extern.substring(position+1));
		...
		public void onGrabbedStateChange(int handler) {
			if (handler != MulitRingView.OnTriggerListener.NO_HANDLE) {
                mCallback.pokeWakelock();
            }
		}
...
//            updateRightTabResources();
    ...
    LockScreen(Context context, Configuration configuration, LockPatternUtils lockPatternUtils,
        ...
        int lockStyle = Settings.System.getInt(context.getContentResolver(),Settings.System.UNLOCK_STYLE,1);
        ...
        //mUnlockWidget.setVisibility(View.GONE);
   ...
   /** private void initRotarySelector() {
        // Rotary setup
//        if (!mRotaryUnlockDown) {
...
//        }
	...
	//        boolean revampedStyle = rotaryStyle == RotaryStyle.getIdByStyle(RotaryStyle.Revamped);
	        ...
	        mRotarySelector.setRotary(!true && true);
	        mRotarySelector.setRevamped(true);
	        mRotarySelector.setLenseSquare(true);// set lock style is lense
...
*/
    private UnlockWidgetCommonMethods createUnlockMethods(View unlockWidget) {
        if (unlockWidget instanceof SlidingTab) {
        	...
        	RotarySelector mRotarySelector = (RotarySelector) unlockWidget;
        	mRotarySelector.enableCustomAppDimple(true);
        	mRotarySelector.setRotary(!true && true);
        	mRotarySelector.setRevamped(true);
 	        mRotarySelector.setLenseSquare(true);// set lock style is lense
 	        ...
 	        RotarySelectorMethods rotarySelectorMethods = new RotarySelectorMethods(mRotarySelector);
 	        mRotarySelector.setOnDialTriggerListener(rotarySelectorMethods);
            return rotarySelectorMethods;
        } else if(unlockWidget instanceof MulitRingView) {	// shendu ring
        	MulitRingView mMulitRingView = (MulitRingView) unlockWidget;
        	MulitRingViewMethods mulitRingViewMethods = new MulitRingViewMethods(mMulitRingView);
            mMulitRingView.setOnTriggerListener(mulitRingViewMethods);
            mStatusViewManager.setExtStatusChange(mulitRingViewMethods);
            return mulitRingViewMethods;
    ...
    /***
     * 设置锁屏壁纸:from lovebizhi app
     * @param context
     * @param layout
     * @return
     */
    static Drawable getLoveBackgroundDrawable(Context context, ViewGroup layout){
    	try {
			Context loveContext = context.createPackageContext("com.lovebizhi.wallpaper", 0);
			String loveWallpaperFile = loveContext.getFilesDir() + "/lockwallpaper";
			if(new File(loveWallpaperFile).exists()){
				Bitmap background = BitmapFactory.decodeFile(loveWallpaperFile);
				return new BitmapDrawable(context.getResources(), background);
			}else{
				return null;
			}
        } catch (NameNotFoundException e) {
        	Log.e("LockScreen", "lovebizhi app is not find...");
			return null;
		}
    }
    
    static Drawable getSettingBackgroundDrawable(Context context, ViewGroup layout){
    	try {
			...
			Log.e("LockScreen", "settings app is not find...");
			return null;
		}
    }
     ...
     * @return
     */
    static boolean isUseLovewallpaper(){
    	  File settingfile = new File("/data/data/com.android.settings/files/lockwallpaper");
          File lovefile = new File("/data/data/com.lovebizhi.wallpaper/files/lockwallpaper");
          if(!lovefile.exists()){
        	  return false;
          }else{
        	  if(settingfile.exists()){
        		  if(settingfile.lastModified() < lovefile.lastModified()){
        			  return true;
        		  }else{
        			  return false;
        		  }
    ...
    static void setBackgroundDrawable(Context context, ViewGroup layout){
        ...
        if (parent != null) {
            ...
            Drawable d = getLoveBackgroundDrawable(context, layout);
            String lockBack = Settings.System.getString(context.getContentResolver(), Settings.System.LOCKSCREEN_BACKGROUND);
            
            if(d == null){
                if (lockBack != null) {
                	d = getSettingBackgroundDrawable(context, layout);
                }
            }else {
              if(!isUseLovewallpaper()){
            	  if (lockBack != null) {
            		  d = getSettingBackgroundDrawable(context, layout);
            	  }
              }
    ...
    static void setBackground(Context context, ViewGroup layout) {
        ...
        if (lockBack != null) {
            if (!lockBack.isEmpty()) {
            	...
            	setBackgroundDrawable(context, layout);
            }
        }else{
