...
public final class CdmaCallTracker extends CallTracker {
    ...
    RegistrantList callLineControlInfoRegistrants = new RegistrantList();
...
//    boolean needsPoll;
    ...
    CdmaCallTracker(CDMAPhone phone) {
    	...
    	cm.registerForLineControlInfo(this, EVENT_CALL_LINE_CONTROL_INFO, null);
    ...
    // cdma call Connect vibration
    public void registerForCallLineControlInfo(Handler h,int what, Object obj) {
    	Registrant localRegistrant = new Registrant(h, what, obj);
    	callLineControlInfoRegistrants.add(localRegistrant);
    }
    
    public void unregisterForCallLineControlInfo(Handler h){
    	callLineControlInfoRegistrants.remove(h);
    }
    ...
    handleMessage (Message msg) {
        ...
        switch (msg.what) {
            ...
            break;
            case EVENT_CALL_LINE_CONTROL_INFO:
                ar = (AsyncResult)msg.obj;
				if (ar.exception == null)
		    		handleCallLineControlInfo((CdmaInformationRecords.CdmaLineControlInfoRec)ar.result);
            break;
    ...
    /*
     * handLineControlInfo
     * cdma connect Vibration
     */
     private void handleCallLineControlInfo(CdmaInformationRecords.CdmaLineControlInfoRec paramCdmaLineControlInfoRec){
		 if (paramCdmaLineControlInfoRec.lineCtrlPolarityIncluded == 1) {
			  Log.i(LOG_TAG, "[CdmaCallTracker]  lineCtrlPolarityIncluded==1");
			  CdmaConnection localCdmaConnection = (CdmaConnection)foregroundCall.getLatestConnection();
			  if (localCdmaConnection != null) {
				Log.i(LOG_TAG, "[CdmaCallTracker]  there is foreground connection!");
				localCdmaConnection.onLineControlInfo();
				Log.d(LOG_TAG, "[CdmaCallTracker] notifyLineControlInfo! ");
				if (callLineControlInfoRegistrants != null)
					callLineControlInfoRegistrants.notifyRegistrants(new AsyncResult(null, null, null));
			  }
		}
     }
