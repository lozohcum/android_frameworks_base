...
import com.android.systemui.statusbar.policy.NetworkListAdapter;
import com.android.systemui.statusbar.policy.NetworkListAdapter.NetworkInfomation;
import com.android.systemui.statusbar.policy.OnSizeChangedListener;
import com.android.systemui.statusbar.policy.NetworkController;
import com.android.systemui.statusbar.policy.NotificationRowLayout;
import com.android.systemui.statusbar.policy.WifiConnectDialog;
import com.android.systemui.statusbar.policy.WifiStateReceiver;
...
public class PhoneStatusBar extends BaseStatusBar {
    ...
    private int mStatusBarBackground = 0;
    ...
    View mNotificationPanel; // the sliding/resizing panel within the
                             // notification window
    ...
    private boolean mCarrierLabelVisible = true;
    private int mCarrierLabelHeight;
    private TextView mEmergencyCallLabel;
    private TextView mWifiStateLabel;
    ...
    PowerWidgetPanel mPowerWidgetPanel;
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.System
                    .getUriFor(Settings.System.STATUS_BAR_BRIGHTNESS_CONTROL),
                    false, this);
            resolver.registerContentObserver(Settings.System
                    .getUriFor(Settings.System.SCREEN_BRIGHTNESS_MODE), false,
                    this);
            resolver.registerContentObserver(Settings.System
                    .getUriFor(Settings.System.STATUS_BAR_BACKGROUND), false,
                    this);
            resolver.registerContentObserver(Settings.System
        ...
        public void update() {
            ...
            mStatusBarBackground = Settings.System.getInt(resolver,
                    Settings.System.STATUS_BAR_BACKGROUND, 1);

            boolean mPowerWidgetEnable = Settings.System.getInt(resolver,
                    Settings.System.EXPANDED_VIEW_WIDGET, 1) == 1;
            SuperViewPager.setPagingEnable(mPowerWidgetEnable);
        }
    }
    ...
    private class ExpandedDialog extends Dialog {
        ExpandedDialog(Context context) {
            super(context,
                    com.android.internal.R.style.Theme_Translucent_NoTitleBar);
        }
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        mNotificationPanelIsFullScreenWidth = (mNotificationPanel
                .getLayoutParams().width == ViewGroup.LayoutParams.MATCH_PARENT);
        mNotificationPanel
                .setSystemUiVisibility(View.STATUS_BAR_DISABLE_NOTIFICATION_TICKER
                        | (mNotificationPanelIsFullScreenWidth ? 0
                                : View.STATUS_BAR_DISABLE_SYSTEM_INFO));

        /*
         * if (!ActivityManager.isHighEndGfx(mDisplay)) {
         * mStatusBarWindow.setBackground(null);
         * mNotificationPanel.setBackground(new FastColorDrawable(context
         * .getResources().getColor(
         * R.color.notification_panel_solid_background))); }
         */
        if (ENABLE_INTRUDERS) {
            mIntruderAlertView = (IntruderAlertView) View.inflate(context,
                    R.layout.intruder_alert, null);
        ...
        mStatusIcons = (LinearLayout) mStatusBarView
        ...
        /*
         * Destroy the old widget before recreating the expanded dialog to make
         * sure there are no context issues
         */
        if (mRecreating) {
        ...
        mPager.setOnPageChangeListener(new ViewPagerOnChangeListener());

        viewLeft = View.inflate(mContext,
                R.layout.status_bar_expanded_notification, null);
        mPageList.add(viewLeft);

        viewRight = View.inflate(mContext,
                R.layout.status_bar_expended_power_widget_panel, null);
        mPageList.add(viewRight);

        mPager.setAdapter(new PagerAdapter(mPageList));

        mPile = (NotificationRowLayout) viewLeft.findViewById(R.id.latestItems);
        ...
        mRotationButton = (RotationToggle) mStatusBarWindow
                .findViewById(R.id.rotation_lock_button);
        ...
        // wifi connection and switch ap
        mWifiViewGroup = (ViewGroup) mStatusBarWindow
                .findViewById(R.id.network_layout);
        mWifiStateLabel = (TextView) mWifiViewGroup
                .findViewById(R.id.network_text);
        WifiStateReceiver receiver = new WifiStateReceiver(mContext,
                mWifiViewGroup);
        receiver.register();

        receiver.mService = this;

        mSettingGroup = (ViewGroup) mStatusBarWindow
                .findViewById(R.id.setting_layout);
        mSettingGroup.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.setClassName("com.android.settings",
                        "com.android.settings.cyanogenmod.PowerWidgetOrderActivity");
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                mContext.startActivity(intent);
                StatusBarManager statusBar = (StatusBarManager) mContext
                        .getSystemService(Context.STATUS_BAR_SERVICE);
                statusBar.collapse();
            }
        });

        WifiLabelOnClickListener wifiListener = new WifiLabelOnClickListener();
        mWifiViewGroup.setOnClickListener(wifiListener);

        mExpandClock = (Clock) mNotificationPanel.findViewById(R.id.clock);

        mScrollView = (ScrollView) viewLeft.findViewById(R.id.scroll);
        mScrollView.setVerticalScrollBarEnabled(false); // less drawing during
                                                        // pulldowns

        mWifiListView = (ListView) mStatusBarWindow.findViewById(R.id.ap_list);
        mWifiListView.setOnItemClickListener(wifiListener);

        mPowerWidget = (PowerWidget) viewLeft.findViewById(R.id.exp_power_stat);
        ...
        mPowerWidgetPanel = (PowerWidgetPanel) viewRight
                .findViewById(R.id.power_widget_panel);
        mPowerWidgetPanel
        ...
        mPowerWidgetPanel.setOnTouchListener(new SuperScrollView.PullUpViewOnTouchListener());

        mTicker = new MyTicker(context, mStatusBarView);

        TickerView tickerView = (TickerView) mStatusBarView
                .findViewById(R.id.tickerText);
        tickerView.mTicker = mTicker;

        mCloseView = (CloseDragHandle) mStatusBarWindow
                .findViewById(R.id.close);
        mCloseView.mService = this;
        mCloseViewHeight = res
        ...
        mBatteryController.addIconView((ImageView) mStatusBarView
                .findViewById(R.id.battery));
        mBatteryController.addLabelView((TextView) mStatusBarView
                .findViewById(R.id.battery_text));
        mNetworkController = new NetworkController(mContext);
        final SignalClusterView signalCluster = (SignalClusterView) mStatusBarView
                .findViewById(R.id.signal_cluster);

        mNetworkController.addSignalCluster(signalCluster);
        signalCluster.setNetworkController(mNetworkController);

        mEmergencyCallLabel = (TextView) viewLeft
                .findViewById(R.id.emergency_calls_only);
        ...
        if (SHOW_CARRIER_LABEL) {
             ...
             * mNetworkController.addMobileLabelView(mCarrierLabel); } else {
    ...
    protected void updateRecentsPanel() {
        ...
        // Make .03 alpha the minimum so you always see the item a bit--
        // slightly below
        // .03, the item disappears entirely (as if alpha = 0) and that
        // discontinuity looks
        // a bit jarring
        mRecentsPanel.setMinSwipeAlpha(0.03f);
        if (mNavigationBarView != null) {
            mNavigationBarView.setListener(mRecentsClickListener,
                    mRecentsPanel, mHomeSearchActionListener);
        }
    }
    ...
    public void showSearchPanel() {
        ...
        WindowManager.LayoutParams lp = (android.view.WindowManager.LayoutParams) mNavigationBarView
                .getLayoutParams();
    ...
    private void addNavigationBar() {
        ...
        WindowManagerImpl.getDefault().addView(mNavigationBarView,
    ...
    private void repositionNavigationBar() {
        ...
        WindowManagerImpl.getDefault().updateViewLayout(mNavigationBarView,
                getNavigationBarLayoutParams());
    }
    ...
    public void addIcon(String slot, int index, int viewIndex,
        ...
        mPowerWidgetPanel.updateAllButtons();
    }
    ...
    public void addNotification(IBinder key, StatusBarNotification notification) {
        /* if (DEBUG) */Slog.d(TAG, "addNotification score="
                + notification.score);
        ...
        try {
            immersive = ActivityManagerNative.getDefault()
                    .isTopActivityImmersive();
            if (DEBUG) {
                Slog.d(TAG, "Top activity is "
                        + (immersive ? "immersive" : "not immersive"));
            }
        } catch (RemoteException ex) {
        }
         ...
         * mIntruderAlertView.applyIntruderContent(notification.notification.
         * intruderView, listener); mCurrentlyIntrudingNotification =
         * notification; // 2. Animate mIntruderAlertView in
         * mHandler.sendEmptyMessage(MSG_SHOW_INTRUDER); // 3. Set alarm to age
         * the notification off (TODO)
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        try {
            ...
            Log.d(TAG,
                    "Reinflation failure for the recents panel, new theme is '"
                            + newConfig.customTheme
                            + "' vs current '"
                            + mContext.getResources().getConfiguration().customTheme
                            + "'");
        }
        // setHeight();
        updateShowSearchHoldoff();
    ...
    protected void updateCarrierLabelVisibility(boolean force) {
        ...
        final boolean makeVisible = !(emergencyCallsShownElsewhere && mNetworkController
                .isEmergencyOnly())
        ...
        if (force || mCarrierLabelVisible != makeVisible) {
            ...
            if (DEBUG) {
                Slog.d(TAG, "making carrier label "
                        + (makeVisible ? "visible" : "invisible"));
            }
    ...
    protected void setAreThereNotifications() {
        ...
        if (DEBUG) {
            Slog.d(TAG,
                    "setAreThereNotifications: N=" + mNotificationData.size()
        ...
        if (mClearButton.isShown()) {
            if (clearable != (mClearButton.getAlpha() == 1.0f)) {
                ObjectAnimator clearAnimation = ObjectAnimator.ofFloat(
                        mClearButton, "alpha", clearable ? 1.0f : 0.0f)
                        .setDuration(250);
        ...
        if (showDot != (nlo.getAlpha() == 1.0f)) {
            ...
            nlo.animate()
                    ...
                    .setListener(
                            showDot ? null : new AnimatorListenerAdapter() {
                                @Override
    ...
    public void showClock(boolean show) {
        ...
        if (mClockView != null) {
            mClockView.setVisibility(show ? (mShowClock ? View.VISIBLE
                    : View.GONE) : View.GONE);
        }
    }
    ...
    public void disable(int state) {
        ...
        if (DEBUG) {
            Slog.d(TAG, String.format(
                    "disable: 0x%08x -> 0x%08x (diff: 0x%08x)", old, state,
                    diff));
        }

        StringBuilder flagdbg = new StringBuilder();
        flagdbg.append("disable: < ");
        flagdbg.append(((state & StatusBarManager.DISABLE_EXPAND) != 0) ? "EXPAND"
                : "expand");
        ...
        flagdbg.append(((diff & StatusBarManager.DISABLE_SYSTEM_INFO) != 0) ? "* "
                : " ");
        flagdbg.append(((state & StatusBarManager.DISABLE_BACK) != 0) ? "BACK"
                : "back");
        flagdbg.append(((diff & StatusBarManager.DISABLE_BACK) != 0) ? "* "
                : " ");
        flagdbg.append(((state & StatusBarManager.DISABLE_HOME) != 0) ? "HOME"
                : "home");
        flagdbg.append(((diff & StatusBarManager.DISABLE_HOME) != 0) ? "* "
                : " ");
        flagdbg.append(((state & StatusBarManager.DISABLE_RECENT) != 0) ? "RECENT"
                : "recent");
        flagdbg.append(((diff & StatusBarManager.DISABLE_RECENT) != 0) ? "* "
                : " ");
        flagdbg.append(((state & StatusBarManager.DISABLE_CLOCK) != 0) ? "CLOCK"
                : "clock");
        ...
        if ((diff & StatusBarManager.DISABLE_SYSTEM_INFO) != 0) {
            ...
            if ((state & StatusBarManager.DISABLE_SYSTEM_INFO) != 0) {
                ...
                mIcons.animate().alpha(1f).translationY(0).setStartDelay(0)
                        .setInterpolator(new DecelerateInterpolator(1.5f))
        ...
        if ((diff & (StatusBarManager.DISABLE_HOME
                | StatusBarManager.DISABLE_RECENT | StatusBarManager.DISABLE_BACK)) != 0) {
    ...
    private void makeExpandedVisible(boolean revealAfterDraw) {
        ...
        WindowManager.LayoutParams lp = (WindowManager.LayoutParams) mStatusBarContainer
                .getLayoutParams();
    ...
    public void animateCollapse(int flags, float velocityMultiplier) {
        if (SPEW) {
            Slog.d(TAG, "animateCollapse(): mExpanded=" + mExpanded
                    ...
                    + mExpanded + " mAnimating=" + mAnimating
                    + " mAnimatingReveal=" + mAnimatingReveal + " mAnimY="
                    + mAnimY + " mAnimVel=" + mAnimVel + " flags=" + flags);
        }
    ...
    void doAnimation(long frameTimeNanos) {
        if (mAnimating) {
            ...
            if (mAnimY < getStatusBarHeight() && mClosing) {
                // Draw one more frame with the bar positioned at the top of the
                // screen
                // before ending the animation so that the user sees the bar in
                // its final position. The call to performCollapse() causes a
                // window
                // relayout which takes time and might cause the animation to
                // skip
                // on the very last frame before the bar disappears if we did it
                // now.
                mAnimY = 0;
                mAnimAccel = 0;
                mAnimVel = 0;
    ...
    void incrementAnim(long frameTimeNanos) {
        final long deltaNanos = Math
                .max(frameTimeNanos - mAnimLastTimeNanos, 0);
    ...
    void doRevealAnimation(long frameTimeNanos) {
        if (SPEW) {
            Slog.d(TAG, "doRevealAnimation: dt="
                    + (frameTimeNanos - mAnimLastTimeNanos));
        }
    ...
    void prepareTracking(int y, boolean opening) {
        if (CHATTY) {
            Slog.d(TAG, "panel: beginning to track the user's touch, y=" + y
                    + " opening=" + opening);
        }
        ...
        if (opening) {
            ...
            if (mAnimating) {
                ...
                mChoreographer.removeCallbacks(
                        Choreographer.CALLBACK_ANIMATION, mAnimationCallback,
                        null);
            }
    ...
    void performFling(int y, float vel, boolean always) {
        ...
        if (mExpanded) {
            if (!always
                    && (vel > mFlingCollapseMinVelocityPx || (y > (getExpandedViewMaxHeight() * (1f - mCollapseMinDisplayFraction)) && vel > -mFlingExpandMinVelocityPx))) {
            ...
            if (always
                    || (vel > mFlingExpandMinVelocityPx || (y > (getExpandedViewMaxHeight() * (1f - mExpandMinDisplayFraction)) && vel > -mFlingCollapseMinVelocityPx))) {
        ...
        if (mBrightnessControl) {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if ((y + mViewDelta) < mNotificationPanelMinHeight) {
                    ...
                    int touchSlop = ViewConfiguration.get(mContext)
                            .getScaledTouchSlop();
    ...
    boolean interceptTouchEvent(MotionEvent event) {
        if (SPEW) {
            Slog.d(TAG, "Touch: rawY=" + event.getRawY() + " event=" + event
                    + " mDisabled=" + mDisabled + " mTracking=" + mTracking);
        } else if (CHATTY) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            if (!mExpanded) {
                ...
                mViewDelta = mAbsPos[1]
                        + getCloseViewHeight() // XXX: not closeViewHeight, but
                                               // paddingBottom from the 9patch
                        + mNotificationPanelBackgroundPadding.top
                        + mNotificationPanelBackgroundPadding.bottom - y;
            }
        }
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if ((!mExpanded && y < hitSize) ||
                ...
                if (x >= edgeBorder
                        && x < mDisplayMetrics.widthPixels - edgeBorder) {
            ...
            if (action == MotionEvent.ACTION_MOVE) {
                if (!mAnimatingReveal
                        || (y + mViewDelta) >= mNotificationPanelMinHeight) {
                    mAnimatingReveal = false;
                    updateExpandedViewPos(y + mViewDelta);
                }
            } else if (action == MotionEvent.ACTION_UP
                ...
                if (CHATTY) {
                    Slog.d(TAG, String.format(
                            "gesture: vraw=(%f,%f) vnorm=(%f,%f) vlinear=%f",
                            mVelocityTracker.getXVelocity(),
                            mVelocityTracker.getYVelocity(), xVel, yVel, vel));
                }

                if (mTrackingPosition == mNotificationPanelMinHeight) {
                    // start the fling from the tracking position, ignore y and
                    // view delta
    ...
    private void setStatusBarLowProfile(boolean lightsOut) {
        if (mLightsOutAnimation == null) {
            final View notifications = mStatusBarView
                    .findViewById(R.id.notification_icon_area);
            final View systemIcons = mStatusBarView
                    .findViewById(R.id.statusIcons);
            final View signal = mStatusBarView
                    .findViewById(R.id.signal_cluster);
            final View signalText = mStatusBarView
                    .findViewById(R.id.signal_cluster_text);
            final View battery = mStatusBarView.findViewById(R.id.battery);
            final View batteryText = mStatusBarView
                    .findViewById(R.id.battery_text);
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        synchronized (mQueueLock) {
            ...
            pw.println("  mExpanded=" + mExpanded + ", mExpandedVisible="
                    + mExpandedVisible + ", mTrackingPosition="
                    + mTrackingPosition);
            pw.println("  mTicking=" + mTicking);
            pw.println("  mTracking=" + mTracking);
            pw.println("  mNotificationPanel="
                    + ((mNotificationPanel == null) ? "null"
                            : (mNotificationPanel + " params=" + mNotificationPanel
                                    .getLayoutParams().debug(""))));
        ...
        if (DUMPTRUCK) {
            synchronized (mNotificationData) {
                ...
                for (int i = 0; i < N; i++) {
                    ...
                    pw.println("    [" + i + "] key=" + e.key + " icon="
                            + e.icon);
                    StatusBarNotification n = e.notification;
                    pw.println("         pkg=" + n.pkg + " id=" + n.id
                            + " score=" + n.score);
                    pw.println("         notification=" + n.notification);
                    pw.println("         tickerText=\""
            ...
            for (int i = 0; i < N; i++) {
                StatusBarIconView ic = (StatusBarIconView) mStatusIcons
                        .getChildAt(i);
    ...
    protected void updateExpandedViewPos(int expandedPosition) {
        setHeight();
        if (SPEW) {
            Slog.d(TAG, "updateExpandedViewPos: expandedPosition="
                    + expandedPosition
                    // + " mTrackingParams.y=" + ((mTrackingParams == null) ?
                    // "?" : mTrackingParams.y)
                    + " mTracking=" + mTracking + " mTrackingPosition="
                    + mTrackingPosition + " mExpandedVisible="
                    + mExpandedVisible + " mAnimating=" + mAnimating
                    + " mAnimatingReveal=" + mAnimatingReveal + " mClosing="
                    + mClosing + " gravity=" + mNotificationPanelGravity);
        }
        int panelh = 0;
        ...
        if (!mExpandedVisible) {
            if (SPEW)
                Slog.d(TAG, "updateExpandedViewPos: view not visible, bailing");
        ...
        if (expandedPosition == EXPANDED_FULL_OPEN) {
            ...
            // panelh = disph;
            // }
            panelh = (expandedPosition <= disph) ? expandedPosition : disph;
        }

        // catch orientation changes and other peculiar cases
        if (panelh > 0
                && ((panelh > disph) || (panelh < disph && !mTracking && !mAnimating))) {
            if (SPEW)
                Slog.d(TAG, "updateExpandedViewPos: orientation change?");
            panelh = disph;
        } else if (panelh < 0) {
            panelh = 0;
        }
        ...
        if (SPEW)
            Slog.d(TAG, "updateExpandedViewPos: adjusting size to panelh="
                    + panelh);

        if (panelh == mTrackingPosition) {
            if (SPEW)
                Slog.d(TAG,
                        "updateExpandedViewPos: panelh == mTrackingPosition, bailing");
            return;
        }
        ...
        if (SPEW) {
            Slog.v(TAG, "updated cropView height=" + panelh + " grav="
                    + lp.gravity);
        }
        ...
        /*
         * if (DIM_BEHIND_EXPANDED_PANEL &&
         * ActivityManager.isHighEndGfx(mDisplay)) { // woo, special effects
         * final float k = (float) (1f - 0.5f * (1f - Math.cos(3.14159f * Math
         * .pow(1f - frac, 2.2f)))); final int color = ((int) (0xB0 * k)) << 24;
    ...
    private View.OnClickListener mClearButtonListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            synchronized (mNotificationData) {
                ...
                final ArrayList<View> snapshot = new ArrayList<View>(
                        numChildren);
                for (int i = 0; i < numChildren; i++) {
                    final View child = mPile.getChildAt(i);
                    if (mPile.canChildBeDismissed(child)
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        for (final View _v : snapshot) {
                            ...
                            currentDelay = Math.max(50, currentDelay
                                    - ROW_DELAY_DECREMENT);
    ...
    private View.OnClickListener mSettingsButtonListener = new View.OnClickListener() {
        public void onClick(View v) {
            // We take this as a good indicator that Setup is running and we
            // shouldn't
    ...
    private void recreateStatusBar() {
        ...
        for (int i = 0; i < nIcons; i++) {
            StatusBarIconView iconView = (StatusBarIconView) mStatusIcons
                    .getChildAt(i);
        ...
        ArrayList<Pair<IBinder, StatusBarNotification>> notifications = new ArrayList<Pair<IBinder, StatusBarNotification>>(
                nNotifs);
        ...
        for (int i = 0; i < nNotifs; i++) {
            Pair<IBinder, StatusBarNotification> notifData = notifications
                    .get(i);
    ...
    protected void loadDimens() {
        ...
        int newIconSize = res
                .getDimensionPixelSize(com.android.internal.R.dimen.status_bar_icon_size);
        int newIconHPadding = res
                .getDimensionPixelSize(R.dimen.status_bar_icon_padding);
        ...
        mSelfCollapseVelocityPx = res
                .getDimension(R.dimen.self_collapse_velocity);
        mFlingExpandMinVelocityPx = res
                .getDimension(R.dimen.fling_expand_min_velocity);
        mFlingCollapseMinVelocityPx = res
                .getDimension(R.dimen.fling_collapse_min_velocity);

        mCollapseMinDisplayFraction = res.getFraction(
                R.dimen.collapse_min_display_fraction, 1, 1);
        mExpandMinDisplayFraction = res.getFraction(
                R.dimen.expand_min_display_fraction, 1, 1);

        mExpandAccelPx = res.getDimension(R.dimen.expand_accel);
        mCollapseAccelPx = res.getDimension(R.dimen.collapse_accel);

        mFlingGestureMaxXVelocityPx = res
                .getDimension(R.dimen.fling_gesture_max_x_velocity);
        ...
        mNotificationPanelMarginLeftPx = (int) res
                .getDimension(R.dimen.notification_panel_margin_left);
        mNotificationPanelGravity = res
                .getInteger(R.integer.notification_panel_layout_gravity);
        if (mNotificationPanelGravity <= 0) {
            mNotificationPanelGravity = Gravity.CENTER_VERTICAL | Gravity.TOP;
        }
        getNinePatchPadding(res.getDrawable(R.drawable.notification_panel_bg),
                mNotificationPanelBackgroundPadding);
        final int notificationPanelDecorationHeight = res
                .getDimensionPixelSize(R.dimen.notification_panel_padding_top)
                + res.getDimensionPixelSize(R.dimen.notification_panel_header_height)
                + mNotificationPanelBackgroundPadding.top
                + mNotificationPanelBackgroundPadding.bottom;
        mNotificationPanelMinHeight = notificationPanelDecorationHeight
                + res.getDimensionPixelSize(R.dimen.close_handle_underlap);

        mCarrierLabelHeight = res
                .getDimensionPixelSize(R.dimen.carrier_label_height);
    ...
    protected boolean shouldDisableNavbarGestures() {
        return mExpanded || NavigationBarView.getEditMode()
    ...
    private void computeDateViewWidth() {
        ...
        try {
            ...
            if (mButtonsBar == null || mClearButton == null
                    || mDateView == null || mDateView.getDoW() == null
                    || mDateView.getDate() == null) {

                return;
            }

            // Obtain the screen width. StatusBar width is not valid on screen
            // rotation
            ...
            float dateViewWidth = mTestPaint.measureText(date.toString()
                    .toUpperCase());

            Paint mTestPaint2 = new Paint();
            mTestPaint2.set(mDateView.getDoW().getPaint());
            float dowViewWidth = mTestPaint2.measureText(mDateView.getDoW().getText()
                    .toString().toUpperCase());

            // Calculate DateView room width
            int maxRoomWidth = width - clearButtonWidth - buttonsBarWidth
                    - mDateView.getLeft();

            // Compute the new width (if text fits in screen show all; otherwise
            // stretch to fit)
            int newWidth = (dateViewWidth > dowViewWidth) ? (int) dateViewWidth : (int) dowViewWidth;
            ...
            if (DEBUG) {
                System.out
                        .println(String
                                .format("width: %s, buttonsBarWidth: %s, clearButtonWidth: %s, "
                                        + "dateViewWidth: %s, maxRoomWidth: %s, newWidth: %s",
                                        width, buttonsBarWidth,
                                        clearButtonWidth, dateViewWidth,
                                        maxRoomWidth, newWidth));
            }
    ...
    private void setHeight() {
        ...
        // set power widget panel height
        android.widget.LinearLayout.LayoutParams panelLayoutParams = (android.widget.LinearLayout.LayoutParams) mPowerWidgetPanel
                .getLayoutParams();
        // height = ScreenHeight - CloseHandler.height - HeaderHeight(something
        // on the top)
        panelLayoutParams.height = getExpandedViewMaxHeight()
                - res.getDimensionPixelSize(R.dimen.close_handle_height)
                - res.getDimensionPixelSize(R.dimen.notification_panel_header_height);
        mPowerWidgetPanel.setLayoutParams(panelLayoutParams);

        // set scroll view height
        android.widget.FrameLayout.LayoutParams lp = (android.widget.FrameLayout.LayoutParams) mScrollView
                .getLayoutParams();
        lp.setMargins(0, 0, 0, 0);
        // height = ScreenHeight - CloseHandler.height - HeaderHeight -
        // PowerWidgetHeight(the bottom)
        lp.height = getExpandedViewMaxHeight()
                - res.getDimensionPixelSize(R.dimen.close_handle_height)
                - res.getDimensionPixelSize(R.dimen.notification_panel_header_height)
                - (mPowerWidget.getHeight());
        mScrollView.setLayoutParams(lp);

        // set power widget position
        lp = (android.widget.FrameLayout.LayoutParams) mPowerWidget
                .getLayoutParams();
        // topMargin = ScreenHeight - CloseHandler.height - HeaderHeight -
        // PowerWidgetHeight(the bottom)
        lp.setMargins(
                0,
                getExpandedViewMaxHeight()
                        - res.getDimensionPixelSize(R.dimen.close_handle_height)
                        - res.getDimensionPixelSize(R.dimen.notification_panel_header_height)
                        - mPowerWidget.getHeight(), 0, 0);
        mPowerWidget.setLayoutParams(lp);
    }
    ...
    protected void visibilityChanged(boolean visible) {
        // TODO Auto-generated method stub
        if (!visible) {
            LayoutParams lp = mWifiListView.getLayoutParams();
            lp.height = 0;
            mWifiListView.setLayoutParams(lp);
            if (mPager != null) {
                mPager.setCurrentItem(0);
            }
        } else if (mExpandClock != null) {
            mExpandClock.updateSettings();
            mExpandClock.updateClock();
        }
        super.visibilityChanged(visible);
    }

    private class WifiLabelOnClickListener implements View.OnClickListener, OnItemClickListener {
        private List<String> apList;
        private List<ScanResult> result;
        private NetworkListAdapter adapter;
        private String mCurrentAp;
        private NetworkListAdapter.NetworkInfomation mNetworkInfomation;
        private final int wifi_icon[] = {
        ...
        private final int wifi_encryption_icon[] = {
                R.drawable.ic_wifi_signal_encryption_1_dark,
                R.drawable.ic_wifi_signal_encryption_2_dark,
                R.drawable.ic_wifi_signal_encryption_3_dark,
                R.drawable.ic_wifi_signal_encryption_4_dark
        };
        ...
        public void onClick(View arg0) {
            ...
            mCurrentAp = wifiState.equals(NetworkInfo.State.CONNECTED) ?
                    manager.getConnectionInfo().getSSID() : "";
            result = manager.getScanResults();
            apList = new ArrayList<String>();
            adapter = new NetworkListAdapter(mContext);

            if (result != null) {
                for (int i = 0; i < result.size(); i++) {
                    // android.util.Log.i("ap_list", apList.get(i).SSID);
                    // TODO:fix the logical here
                    if (result.get(i).level != Integer.MAX_VALUE) { // not out
                                                                    // of range
                        apList.add(result.get(i).SSID);
                        mNetworkInfomation = new NetworkInfomation();
                        mNetworkInfomation.mTitleText = result.get(i).SSID;
                        mNetworkInfomation.mLevelDrawable =
                                // mContext.getResources().getDrawable(wifi_icon[WifiManager.calculateSignalLevel(result.get(i).level,
                                // 4)]);
                                mContext.getResources()
                                        .getDrawable(
                                                WifiAuthentication.getSecurityType(result.get(i)) == WifiAuthentication.SECURITY_NONE ?
                                                        wifi_icon[WifiManager.calculateSignalLevel(
                                                                result.get(i).level, 4)]
                                                        :
                                                        wifi_encryption_icon[WifiManager
                                                                .calculateSignalLevel(
                                                                        result.get(i).level, 4)]);
                        if (mCurrentAp.equals(result.get(i).SSID) && !flag) {
                            flag = true; //don't add "connected" again
                            mNetworkInfomation.mConnectText =
                                    mContext.getResources().getString(R.string.wifi_connected);
                            mNetworkInfomation.mLevelDrawable = mContext.getResources()
                                    .getDrawable(R.drawable.ic_wifi_signal_blue_dark);
                        } else {
                            mNetworkInfomation.mConnectText = "";
                        }
                        adapter.addItem(mNetworkInfomation);
                    }
                }
                mWifiListView.setAdapter(adapter);
                lp.height = mContext.getResources().getDimensionPixelSize(
                        R.dimen.network_list_height)
                        * (mWifiListView.getAdapter().getCount() > 5 ? 5 : mWifiListView
                                .getAdapter().getCount());
                mWifiListView.setLayoutParams(lp);
                Animation ani = AnimationUtils.loadAnimation(mContext, R.anim.wifi_menu_in);
                mWifiListView.setAnimation(ani);
                mWifiListView.startAnimation(ani);
                // mWifiListView.setBackgroundColor(android.R.color.white);
            }
        }
        ...
        public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
                long arg3) {
            // TODO Auto-generated method stub
            if (mWifiStateLabel.getText().toString().equals(apList.get(arg2))) {
                return;
            }
            WifiManager manager = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
            WifiAuthentication auth = new WifiAuthentication();
            WifiConfiguration config = WifiAuthentication.IsExsits(mContext, apList.get(arg2));
            if (config != null) {
                manager.enableNetwork(manager.addNetwork(config), true);
            } else {
                WifiConnectDialog act = new WifiConnectDialog();
                Intent intent = new Intent(mContext, act.getClass());
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                intent.putExtra("SSID", apList.get(arg2));
                mContext.startActivity(intent);
            }
            // manager.enableNetwork(arg0, true);
            animateCollapse();
        }
    }
    ...
    public class ViewPagerOnChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            ...
            switch (position) {
                case 0:
                    mWifiViewGroup.setVisibility(View.VISIBLE);
                    mSettingGroup.setVisibility(View.GONE);
                    break;
                case 1:
                    mWifiViewGroup.setVisibility(View.GONE);
                    mSettingGroup.setVisibility(View.VISIBLE);
            }
        }
    ...
    public static class WifiAuthentication {
        ...
        public static WifiConfiguration IsExsits(Context context, String SSID) {
            ...
            // check if no need to enter password
            mWifiManager.startScan();
            List<ScanResult> result = mWifiManager.getScanResults();
            for (int i = 0; i < result.size(); i++) {
                if (result.get(i).SSID.equals(SSID)
                        && getSecurityType(result.get(i)) == SECURITY_NONE) {
                    return CreateWifiInfo(SSID, "", SECURITY_NONE);
                }
            }
            return null;
        }
    ...
    public void closeWifiMenu() {
        Animation ani = AnimationUtils.loadAnimation(mContext, R.anim.wifi_menu_out);
        mWifiListView.setAnimation(ani);
        mWifiListView.startAnimation(ani);
