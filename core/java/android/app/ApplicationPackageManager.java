...
final class ApplicationPackageManager extends PackageManager {
     ...
     *  
     *  @author liuyongsheng
     *  @param activityName : intent is launcher main Activity
     *  @param packageName : apk package name
     *  @param resid : request drawable is id
    ...
    /**@hide*/
    @Override
    public Drawable shenduGetDrawable(String activityName, String packageName, int resid,
            ApplicationInfo appInfo) {  
        ResourceName name = new ResourceName(packageName, resid);
        Drawable dr = getCachedIcon(name);
        if (dr != null) {
        ...
        try {
            ...
            if(activityName != null){
                dr = r.shenduGetActIconDrawable(activityName,resid);
            }else{
                dr = r.shenduGetPkgIconDrawable(packageName,resid);
            }
            if (DEBUG_ICONS) Log.v(TAG, "Getting drawable 0x"
                    + Integer.toHexString(resid) + " from " + r
                    + ": " + dr);
    ...
    @Override public Drawable getDefaultActivityIcon() {
    	 Drawable d = Resources.getSystem().shenduGetSymDefAppIconDrawable("sym_def_app_icon", com.android.internal.R.drawable.sym_def_app_icon);
         if(d!=null){
             return d;
         }
	...
	public String[] getRevokedPermissions(String packageName) {
		try {
    		//song
		    //mContext.enforceCallingOrSelfPermission(android.Manifest.permission.REVOKE_PERMISSIONS, null);
            //song
