 ...
 * as the activity manager requests. {@hide}
 */
public final class ActivityThread {
    ...
    // holds their own lock. Thus you MUST NEVER call back into the activity
    // manager
    // or window manager or anything that depends on them while holding this
    // lock.
    ...
    final HashMap<ComponentName, ProviderClientRecord> mLocalProvidersByName = new HashMap<ComponentName, ProviderClientRecord>();
    ...
    private class H extends Handler {
        ...
        private void maybeSnapshot() {
            if (mBoundApplication != null && SamplingProfilerIntegration.isEnabled()) {
                // convert the *private* ActivityThread.PackageInfo to *public*
                // known
                ...
                SamplingProfilerIntegration.writeSnapshot(mBoundApplication.processName,
                        packageInfo);
            }
        }
    }
    ...
    public static IPackageManager getPackageManager() {
        if (sPackageManager != null) {
            // Slog.v("PackageManager", "returning cur default = " +
            // sPackageManager);
    ...
    Resources getTopLevelResources(String resDir, CompatibilityInfo compInfo) {
        ...
        /*
         * Attach theme information to the resulting AssetManager when
         * appropriate.
         */
        ...
        if (pi != null && pi.applicationInfo != null && pi.themeInfos != null) {
            ...
            if (cookie != 0) {
                ...
                for (int i = 0; i < N; i++) {
                    ...
                    try {
                        PackageRedirectionMap map = rm.getPackageRedirectionMap(themePackageName,
                                themeId,
                                packageName);
                        if (map != null) {
                        ...
                        Log.e(TAG,
                                "Failure accessing package redirection map, removing theme support.");
    ...
    public final LoadedApk getPackageInfo(String packageName, CompatibilityInfo compatInfo,
        ...
        synchronized (mPackages) {
            ...
            // Slog.i(TAG, "getPackageInfo " + packageName + ": " +
            // packageInfo);
            // if (packageInfo != null) Slog.i(TAG, "isUptoDate " +
            // packageInfo.mResDir
    ...
    public final ActivityClientRecord performResumeActivity(IBinder token,
        ...
        ActivityClientRecord r = mActivities.get(token);
        if (localLOGV)
            Slog.v(TAG, "Performing resume of " + r
    ...
    final void handleResumeActivity(IBinder token, boolean clearHide, boolean isForward) {
        ...
        if (r != null) {
            ...
            if (!r.activity.mFinished && willBeVisible
                ...
                if (localLOGV)
                    Slog.v(TAG, "Resuming " + r + " with isForward="
                            + isForward);
                WindowManager.LayoutParams l = r.window.getAttributes();
                if ((l.softInputMode
    ...
    private Bitmap createThumbnailBitmap(ActivityClientRecord r) {
        ...
        try {
            if (thumbnail == null) {
                ...
                // On platforms where we don't want thumbnails, set dims to
                // (0,0)
                if ((w > 0) && (h > 0)) {
     ...
     * because the server's meaning of stop is slightly different than our
     * client -- for the server, stop means to save state and give it the result
     * when it is done, but the window may still be visible. For the client, we
     * want to call onStop()/onStart() to indicate when the activity's UI
     * visibillity changes.
     */
    ...
    private void handleDestroyActivity(IBinder token, boolean finishing,
        ...
        if (r != null) {
            ...
            if (r.mPendingRemoveWindow == null) {
                ...
                // using this recreate facility then they get to live with
                // leaks.
            ...
            for (ActivityClientRecord ar : mActivities.values()) {
                ...
                if (a != null) {
                    ...
                    if (!ar.activity.mFinished && (allActivities || !ar.paused)) {
                        // If the activity is currently resumed, its
                        // configuration
    ...
    final int applyConfigurationToResourcesLocked(Configuration config,
        ...
        // boolean systhemeChanged = (changes &
        // ActivityInfo.CONFIG_THEME_RESOURCE) != 0;
        // if(systhemeChanged){
        // Log.i("LYS-DEMO",
        // "*********===================***&&&&&&&&&&&&&&&&&&&&&&&&");
        // Resources.getSystem().clear();
        // Resources.getSystem().cleanZipfiles();
        // Resources.getSystem().load();
        // }
    ...
    final void freeTextLayoutCachesIfNeeded(int configDiff) {
        if (configDiff != 0) {
            // Ask text layout engine to free its caches if there is a locale
            // change
    ...
    final void handleLowMemory() {
        ...
        // Ask SQLite to free up as much memory as it can, mostly from its page
        // caches.
    ...
    private void handleBindApplication(AppBindData data) {
        ...
        /*
         * Before spawning a new process, reset the time zone to be the system
         * time zone. This needs to be done because the system time zone could
         * have changed after the the spawning of this process. Without doing
         * this this process would have the incorrect system time zone.
         */
        TimeZone.setDefault(null);

        /*
         * Initialize the default locale in this process for the reasons we set
         * the time zone.
         */
         ...
         * reflect configuration changes. The configuration object passed in
         * AppBindData can be safely assumed to be up to date
         */
        ...
        /**
         * For system applications on userdebug/eng builds, log stack traces of
         * disk and network access to dropbox for analysis.
         */
        ...
        /**
         * For apps targetting SDK Honeycomb or later, we don't allow network
         * usage on the main event loop / UI thread. Note to those grepping:
         * this is what ultimately throws NetworkOnMainThreadException ...
         */
        ...
        if (b != null) {
            ...
            // all system is up. This includes the connectivity service, so
            // don't
        ...
        try {
            ...
            // don't bring up providers in restricted mode; they may depend on
            // the
            ...
            // Do this after providers, since instrumentation tests generally
            // start their
    ...
    public final boolean releaseProvider(IContentProvider provider, boolean stable) {
        ...
        synchronized (mProviderMap) {
                ...
                if (prc.unstableCount == 0) {
                    ...
                    if (!lastRef) {
                        try {
                            ...
                            // do nothing content provider object is dead any
                            // way
                        }
                    }
                }
            ...
            if (lastRef) {
                if (!prc.removePending) {
                    // Schedule the actual remove asynchronously, since we don't
                    // know the context
    ...
    /**
     * Installs the provider. Providers that are local to the process or that
     * come from the system server may be installed permanently which is
     * indicated by setting noReleaseNeeded to true. Other remote providers are
     * reference counted. The initial reference count for all reference counted
     * providers is one. Providers that are not reference counted do not have a
     * reference count (at all). This method detects when a provider has already
     * been installed. When this happens, it increments the reference count of
     * the existing provider (if appropriate) and returns the existing provider.
     * This can happen due to concurrent attempts to acquire the same provider.
     */
