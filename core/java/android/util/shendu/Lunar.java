...
public class Lunar {
    ...
    private String[] oneToten = new String[] {"一","二","三","四","五","六","七","八","九","十"};
    ...
    public  String getChinaDayString(boolean md, int day) {
        String chineseTen[] = mContext.getResources().getStringArray(R.array.chineseten);
        String monthString = mContext.getResources().getString(R.string.status_month);
        int n = day % 10 == 0 ? 9 : day % 10 - 1;
        if (day > 30)
            return "";
        if (md == false)
            if (day == 1)
                return chineseNumber[month - 1] + monthString;
        if (day == 10)
            ...
            return chineseTen[day / 10] + oneToten[n];
    }

    public String toString(boolean md) {
        ...
        return animalsYear() + year1 + (leap ? run1 : "") + chineseNumber[month - 1] + month1
                + getChinaDayString(md, day);
    }
}
